...
void writeToCSVfile(MatrixXd& m, string fname);

// define the format you want, you only need one instance of this... added 6/5/2020
// void writeToCSVfile(string fname,MatrixXd& m);
...
// eigen path name: /Users/joshstern/Downloads/eigen-3.3.7/Eigen
...
// 6/1/20 JS Leave off here (where the fun stuff starts happening)
...
int main(int argc, char* argv[])
{
    ...
    int RANDW = 0;
    if (argc > 1)
        // just taking arguments from commandline
    ...
    int NBTRIALS = 10000 ; //100407; // ~10K trials sufficient to get good convergence (95% correct on a binary criterion is reached within ~1000 trials, but performance keeps improving after that). Should really be 100K if you have time.
    int TRIALTIME = 1000;
    ...
    // initialize matrices for patterns and target responses
    ...
    win.setRandom();
    win.row(0).setZero(); // Input weights are uniformly chosen between -1 and 1, except possibly for output cell (not even necessary). No plasticity for input weights.
...
//cout << J(0,0) << " " << win(1,1) << endl;
    ...
    // note that we didn't init some of these- probably won't hurt us in Julia though
    ...
    for (int numtrial=0; numtrial < NBTRIALS; numtrial++)
    {
        ...
        if (PHASE == LEARNING)
        {
            ...
            {
                cout << "-----\n";
                cout <<  "Trial " << numtrial << "- trial type: " << trialtype;
                //cout << ", responses : " << zout;
                ...
                cout << "J(0,1:4): " << J.row(0).head(4) << "\n" ;
                cout << "dJ(0,1:4): " << dJ.row(0).head(4) << "\n" ;
                cout << "Relative error: " << (meanerr - meanerrtrace(trialtype)) * meanerrtrace(trialtype) << "\n";
                // cout << endl;
                cout << "-----\n";
            }
...
void writeToCSVfile(MatrixXd& J,string fname)
{
  ofstream file(fname.c_str());
  for(int  i = 0; i < J.rows(); i++){
        for(int j = 0; j < J.cols(); j++){
           string str = to_string(J(i,j));
           if(j+1 == J.cols()){
               file<<str;
           }else{
               file<<str<<',';
